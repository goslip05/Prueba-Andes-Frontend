...
import userImage from '../../assets/images/user_logo.png';

export default function Topbar({ logout, user }) {
    ...
    <>
      <nav className="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
        ...
        <button id="sidebarToggleTop" className="btn btn-link d-md-none rounded-circle mr-3">
            <i className="fa fa-bars" onClick={() => handleClickSidebarToggle($)}></i>
        </button>
        ...
        <ul className="navbar-nav ml-auto">
            ...
            <li className="nav-item dropdown no-arrow mr-4">
                <a className="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span className="mr-2 d-none d-lg-inline text-gray-600 small text-right">
                        {user ? user.name || "Hola Usuario, Bienvenido" : "Cargando..."}
                    </span>
                    <img className="img-profile rounded-circle" src={userImage} />
                </a>
                <div className="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                    
                    <button className="dropdown-item" id="boton_cerrar_sesion" onClick={() => handleClickLogout()}>
